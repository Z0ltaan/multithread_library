...
namespace foo
{
    ...
    std::shared_ptr< mt::threadsafe_logger::output > output_ptr)
  {
    foo::message dummy{};
    std::string empty{ "" };
    mt::threadsafe_logger logger(
      "Consumer", output_ptr, mt::threadsafe_logger::LOG_LEVEL::INFO);
  ...
  {
    mt::threadsafe_logger logger(
      "Supplier", output_ptr, mt::threadsafe_logger::LOG_LEVEL::INFO);
    for (size_t i = 0; i < iterations; ++i)
    {
  ...
  void launch_threads(std::vector< std::thread >& pool,
                      size_t thread_count,
                      Callable thread_func,
                      Args&&... args)
  {
  ...
  {
    std::for_each(
      pool.begin(), pool.end(), [](auto& thread) { thread.join(); });
  }
...
main(int argc, char** argv)
{
  ...
  std::ofstream out(foo::get_program_dir(argv[0]) / "log.txt");
  {
    std::shared_ptr< mt::threadsafe_logger::output > output_ptr =
      std::make_shared< mt::threadsafe_logger::output >(out);
    launch_threads(suppliers,
                   supplier_count,
                   foo::supplier_func,
                   iterations_count,
                   queue_ptr,
                   output_ptr);
    launch_threads(consumers,
                   consumer_count,
                   foo::consumer_func,
                   iterations_count,
                   queue_ptr,
